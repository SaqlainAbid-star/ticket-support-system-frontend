...
export default function Tickets() {
  ...
  const fetchTickets = async () => {
    try {
      ...
      //   `http://localhost:3001/api/getCustomerTickets/${_id}`,
      //   {
      //     method: "GET",
      //     headers: {
      ...
      const res = await fetch("https://ticket-support-system-backend-4u1x.vercel.app/api/getAllTickets", {
        method: "GET",
      });
  ...
  const searchCustomer = () => {
    ...
    if (search !== '') {
      setFilterCustomerTickets(
        filterCustomerTickets.filter(i => i.customerName.toLowerCase().includes(search.toLowerCase()))
      )
